...
const ACTIVE_ITEMS = {
	...
	"方块": "res://sence/equipment/装备/主动道具/主动装备库/方块/block_template.tscn",
	"发射器": "res://sence/equipment/装备/主动道具/主动装备库/方块/dispenser_template.tscn",
}
const AI_OPTIONS = {
...
var _bottom_ui_layer: CanvasLayer = null
...
func _ready():
	...
	# 生成测试建筑（敌方阵营，地图中央）
	_spawn_test_building()

func _spawn_test_building():
	var ENEMY_TEMPLATE = load("res://sence/Fight/建筑父对象/建筑子类/enemy_main_building.tscn")
	var PLAYER_TEMPLATE = load("res://sence/Fight/建筑父对象/建筑子类/player_main_building.tscn")

	var bld
	# 敌方：建筑x=350，刷新点在x=380（后方30px）
	if ENEMY_TEMPLATE:
		bld = ENEMY_TEMPLATE.instantiate()
		bld.camp = 1
		bld.position = Vector2(350, 0)
		bld.scale = Vector2(3, 3)
		add_child(bld)
	# 我方：建筑x=-350，刷新点在x=-380（后方30px）
	if PLAYER_TEMPLATE:
		bld = PLAYER_TEMPLATE.instantiate()
		bld.camp = 0
		bld.position = Vector2(-350, 0)
		bld.scale = Vector2(3, 3)
		add_child(bld)
...
func _spawn_from_file(tscn_path: String, camp_id: int):
	...
	var x = -380 if camp_id == 0 else 380
	# TODO: 未来可恢复 y 轴三路分层：soldier.position = Vector2(x, [-1.0, 0.0, 1.0][randi() % 3]); soldier._tier = tier
	soldier.position = Vector2(x, -3.0)
	soldier._tier = 0
	soldier.scale = Vector2(3, 3)
